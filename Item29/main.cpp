...
#endif
private:
    struct StringValue  // 将真正的引用计数和数据耦合在一起
    {
...
public:
    RCObject( )
        : refCount( 0 ),    // 为什么 refCount 初始化为 0？
          shareable( true )
    {
    }
...
private:
    ...
    RCPtr<StringValue> value;    // 自动执行引用计数逻辑
};
...
{
public:
    Widget( int size ) { std::cout << "Widget constructor" << std::endl; }
    void doThis( ) { std::cout << "Widget doThis" << std::endl; }
    int  showThat( ) const
    {
        std::cout << "Widget showThat" << std::endl;
        return 0;
    }
};
